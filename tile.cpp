			...
			if (compressed.size() > max_tile_size && !prevent[P_KILOBYTE_LIMIT]) {
				...
				if (pass == 1) {
					...
					if (outdb != NULL) {
						...
						pmtilesv3_write_tile(outfile, z, tx, ty, compressed.data(), compressed.size());
					}
